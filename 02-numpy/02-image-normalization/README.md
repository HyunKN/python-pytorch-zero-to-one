...
| 파일 | 구현 내용 |
|---|---|
| [exercise.py](exercise.py) | 기본 — 가상 배열로 `normalize_image()` 구현 |
| [exercise_pil.py](exercise_pil.py) | Pillow로 `sample.png`를 불러와 `normalize_image()` 구현 |
| [exercise_cv2.py](exercise_cv2.py) | OpenCV로 불러오기 + BGR→RGB 변환 + `normalize_image()` 구현 |
