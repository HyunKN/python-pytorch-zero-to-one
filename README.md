...
각 문제에서 사용자는 `solution.py`의 정답 함수만 작성합니다. IDE에서 `solution.py`를 열고 Python Run 버튼을 누르면 예제 입력이 자동으로 전달되고 다음 항목이 출력됩니다.
...
자동 검증 구조는 [LeetCode exercise format](docs/leetcode-exercise-format.md)에 기록합니다.
...
- [LeetCode exercise format](docs/leetcode-exercise-format.md)
